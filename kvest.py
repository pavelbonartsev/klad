...
import random 
import datetime
questions = ["При каком дискриминанте квадратное уравнение имеет решение?", "Что такое средняя линия треугольника?",
             "Какой командой в языке Python вывести данные на экран?", "По какому принципу вычисляется напряженность \
электрического поля?", "Коэффициент подобия?", "Какой командой вставить объект в массив по определенному индексу", "Цвет обложки учебника по геометрии?"]
answers = [["1. При положительном", "2. При положительном или нулевом", "3. При отрицательном"], ["1. Отрезок, соединяющий \
середины его сторон", "2. Отрезок, соединяющий вершину треугольника с точкой на противоположной стороне и делящий угол треугольника пополам",
"3. Одна из сторон треугольника"], ["1. pop()", "2. append()", "3. print()", "4. str()"], ["1. Принцип симметрии", "2. Принцип наложения",
"3. Принцип суперпозиции"], ["1. Отношение сходственных сторон подобных треугольников", "2. Отношение соседних сторон треугольника",
"3. Отношение диагонали квадрата к его стороне"], ["1. extend()", "2. append()", "3. add()", "4. insert()"], ["1. Зеленая", "2. Белая",
"3. Красная"]]
right, ok = [2, 1, 3, 3, 1, 4, 2], 0
...
def store():
    ...
    while choice != "Хватит":
        ...
        for i in range(len(products)):
            ...
            if i == 0:
                print(
                    "Позиция   " + "Товар" + " " * (len(max(products, key=len)) - len("Товар") + 20) + 
                    "Стоимость" + " " * 15 + "Осталось", end="\n\n")
            print(
                str(i + 1) + "." + " " * 8 + products[i] + " " * (len(max(products, key=len)) - len(products[i]) + 20) + 
            ...
            s = 0
        print()
        print("Вводите в формате - количество товара (неотрицательное число) - пробел - \
...
input()
clear()
print("'Нет, конечно, можно и самому этот тест разгадывать, но зачем, вдруг есть подсказки?' \
    ...
    while fl == 0:
        while inventary[4] == 0:
            ...
            if inventary[4] == 0:
                ...
                if fl == 1:
                    clear()
                    print("Можно без сомнения сказать, что обложка учебника по геометрии белая.")
                    input()
                    clear()
                    break
if a == "2" or a == "1":
